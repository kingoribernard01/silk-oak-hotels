...
import { Star, Coffee, Wifi, Car, Monitor, Trophy, GlassWater, Bed, MapPin, Phone, Mail, Calendar, CheckCircle, Facebook, Instagram, Twitter, ArrowRight, MessageCircle } from 'lucide-react';
...
const FloatingWhatsApp = () => (
  <a 
    ...
    className="fixed bottom-6 right-6 z-50 bg-[#25D366] hover:bg-[#20bd5a] text-white p-4 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 group flex items-center justify-center animate-bounce"
    style={{ animationDuration: '3s' }}
    aria-label="Chat on WhatsApp"
  >
    <MessageCircle className="w-8 h-8 fill-current" />
    <span className="absolute right-full mr-3 bg-white text-charcoal-900 px-3 py-1 rounded-lg text-sm font-bold shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap pointer-events-none">
      Chat with us
    </span>
  </a>
);
...
const SectionHeading = ({ title, subtitle, light = false }: { title: string, subtitle?: string, light?: boolean }) => (
  <div className="text-center mb-16 px-4">
    <div className={`inline-block mb-3 text-xs font-bold tracking-[0.2em] uppercase ${light ? 'text-gold-400' : 'text-gold-600'}`}>
      Discover Silk Oak
    </div>
    <h2 className={`text-4xl md:text-5xl font-serif font-bold mb-6 ${light ? 'text-white' : 'text-charcoal-900'}`}>{title}</h2>
    {subtitle && <div className={`w-16 h-1 mx-auto rounded-full mb-6 ${light ? 'bg-gold-500' : 'bg-gold-500'}`}></div>}
...
const FeatureIcon = ({ icon: Icon, title }: { icon: any, title: string }) => (
  <div className="flex flex-col items-center p-8 text-center group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 border border-gray-100 hover:border-gold-200 hover:-translate-y-2">
    <div className="w-20 h-20 rounded-full bg-beige-100 flex items-center justify-center mb-6 group-hover:bg-gold-500 transition-colors duration-500">
      <Icon className="w-8 h-8 text-charcoal-800 group-hover:text-white transition-colors duration-500" strokeWidth={1.5} />
    </div>
    <h3 className="text-xl font-semibold font-serif text-charcoal-900 group-hover:text-gold-600 transition-colors">{title}</h3>
  </div>
);

const LocationCard = ({ name, phone, email, image, delay }: { name: string, phone: string, email: string, image: string, delay: number }) => (
  <FadeIn delay={delay} className="h-full">
    <div className="group h-full bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 flex flex-col border border-gray-100">
      <div className="relative h-72 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 z-20 bg-white/95 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider text-charcoal-900 shadow-sm border border-gray-100">
          Open Now
        </div>
      </div>
      <div className="p-8 flex-grow flex flex-col">
        <h3 className="text-2xl font-serif font-bold text-charcoal-900 mb-2 group-hover:text-gold-600 transition-colors">{name}</h3>
        ...
        <div className="mt-8 pt-6 border-t border-gray-100">
           <a href={`tel:${phone.replace(/\s/g, '')}`} className="flex items-center justify-center w-full py-4 px-6 bg-charcoal-900 text-white hover:bg-gold-500 rounded-xl transition-all duration-300 font-bold uppercase text-xs tracking-widest group/btn shadow-lg hover:shadow-gold-500/30">
...
const AmenityCard = ({ icon: Icon, title, description, delay }: { icon: any, title: string, description: string, delay: number }) => (
  <FadeIn delay={delay} className="h-full">
    <div className="h-full p-8 bg-white hover:bg-beige-50 rounded-2xl border border-gray-100 hover:border-gold-200 transition-all duration-500 hover:-translate-y-2 shadow-sm hover:shadow-xl group">
      <div className="w-14 h-14 rounded-full bg-gold-50 text-gold-600 flex items-center justify-center mb-6 group-hover:bg-gold-500 group-hover:text-white transition-colors duration-500">
        <Icon className="w-7 h-7" strokeWidth={1.5} />
      </div>
      <h4 className="text-xl font-serif font-bold text-charcoal-900 mb-3 group-hover:text-gold-600 transition-colors">{title}</h4>
...
const PackageCard = ({ title, features, featured = false, price, delay }: { title: string, features: string[], featured?: boolean, price?: string, delay: number }) => (
  <FadeIn delay={delay} className="h-full">
    <div className={`relative h-full p-8 md:p-10 rounded-3xl border transition-all duration-500 flex flex-col ${featured ? 'bg-charcoal-900 text-white border-gold-500 shadow-2xl scale-100 md:scale-105 z-10' : 'bg-white text-charcoal-900 border-gray-100 shadow-xl hover:shadow-2xl hover:-translate-y-1'}`}>
      {featured && (
        <div className="absolute top-0 right-0 left-0 flex justify-center -mt-4">
          <span className="bg-gold-500 text-white text-xs font-bold px-4 py-2 rounded-full uppercase tracking-widest shadow-lg">Most Popular</span>
        </div>
      )}
      
      <div className="mb-6 text-center">
        <h3 className={`text-2xl font-serif font-bold mb-2 ${featured ? 'text-white' : 'text-charcoal-900'}`}>{title}</h3>
        {price && <p className={`text-sm opacity-60 uppercase tracking-widest ${featured ? 'text-gold-200' : 'text-gray-500'}`}>{price}</p>}
      </div>
      ...
      <ul className="space-y-5 mb-10 flex-grow">
        {features.map((feature, i) => (
          <li key={i} className="flex items-start">
            ...
            <span className={`text-sm md:text-base ${featured ? 'text-gray-300' : 'text-gray-600'}`}>{feature}</span>
          </li>
        ))}
      </ul>
      
      <button className={`w-full py-4 rounded-xl font-bold uppercase tracking-wide text-sm transition-all duration-300 shadow-lg transform hover:scale-[1.02] ${featured ? 'bg-gradient-to-r from-gold-500 to-gold-600 text-white hover:shadow-gold-500/20' : 'bg-charcoal-900 text-white hover:bg-charcoal-800 hover:shadow-xl'}`}>
        Choose Package
      </button>
    </div>
  </FadeIn>
      ...
      <section id="about" className="py-24 md:py-32 container mx-auto px-4 md:px-8">
        <FadeIn>
          <div className="max-w-4xl mx-auto text-center mb-20 relative">
            ...
            <p className="text-3xl md:text-4xl lg:text-5xl font-serif font-medium text-charcoal-900 leading-tight relative z-10">
              “Silk Oak Hotels brings together <span className="italic text-gold-600">comfort</span>, <span className="italic text-gold-600">convenience</span> and <span className="italic text-gold-600">modern hospitality</span> across three prime Nairobi locations.”
            </p>
          </div>
        </FadeIn>
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
            ...
            <LocationCard 
              ...
              // Updated to a cityscape/building image to better represent CBD location
        ...
        <div className="container mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4 h-[1000px] md:h-[600px]">
             <FadeIn delay={0} className="md:col-span-2 md:row-span-2 h-full">
               <div className="relative h-full overflow-hidden rounded-2xl group cursor-pointer border border-white/5">
                 ...
                 <div className="absolute bottom-6 left-6 z-20 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                    <p className="font-serif text-2xl font-bold">Evening Ambiance</p>
                    <p className="text-gold-400 text-sm tracking-widest uppercase">Luxury & Calm</p>
                 </div>
               </div>
             </FadeIn>
             ...
             <FadeIn delay={400} className="md:col-span-2 md:row-span-1 h-full">
               <div className="relative h-full overflow-hidden rounded-2xl group cursor-pointer border border-white/5">
                  ...
                  <img src="https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?q=80&w=1000&auto=format&fit=crop" className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-1000" alt="Cocktails" />
                   <div className="absolute bottom-6 left-6 z-20 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                    <p className="font-serif text-xl font-bold">Signature Cocktails</p>
                 </div>
               </div>
             </FadeIn>
          </div>
          ...
          <div className="bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col lg:flex-row border border-gray-100">
            ...
            <div className="p-10 md:p-16 lg:w-7/12 bg-white">
              <div className="flex items-center justify-between mb-8">
                ...
                <span className="px-3 py-1 bg-gold-100 text-gold-700 text-xs font-bold uppercase rounded-full tracking-wider">Online Request</span>
              </div>
              ...
              <form className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" placeholder="John Doe" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 transition-all placeholder:text-gray-300" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-charcoal-600 uppercase tracking-wider">Phone Number</label>
                    <input type="tel" placeholder="+254 7..." className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 transition-all placeholder:text-gray-300" />
                  </div>
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-bold text-charcoal-600 uppercase tracking-wider">Select Location</label>
                    <div className="relative">
                      <select className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 transition-all text-gray-700 appearance-none cursor-pointer">
                ...
                <div className="space-y-2">
                    ...
                    <textarea rows={4} placeholder="I would like to book a double room for..." className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 transition-all placeholder:text-gray-300 resize-none"></textarea>
                </div>
                
                <button type="button" className="w-full bg-charcoal-900 hover:bg-gold-500 hover:text-white text-white font-bold py-5 rounded-xl uppercase tracking-widest transition-all duration-300 shadow-xl hover:shadow-2xl hover:shadow-gold-500/20 transform hover:-translate-y-1">
      ...
      <footer className="bg-charcoal-950 text-white pt-24 pb-12 border-t border-white/5 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
             <div className="lg:col-span-1">
                <div className="flex items-center gap-3 mb-8">
                  <div className="w-10 h-10 bg-gold-500 rounded flex items-center justify-center text-charcoal-900 font-serif font-bold text-xl shadow-[0_0_15px_rgba(212,175,55,0.4)]">S</div>
                ...
                <div className="flex space-x-4">
                  <a href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-gold-500 hover:text-white transition-all duration-300 group"><Facebook className="w-5 h-5 transform group-hover:scale-110 transition-transform" /></a>
                  <a href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-gold-500 hover:text-white transition-all duration-300 group"><Instagram className="w-5 h-5 transform group-hover:scale-110 transition-transform" /></a>
                  <a href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-gold-500 hover:text-white transition-all duration-300 group"><Twitter className="w-5 h-5 transform group-hover:scale-110 transition-transform" /></a>
                </div>
             </div>
             ...
             <div>
                <h4 className="text-sm font-bold uppercase tracking-widest mb-8 text-white">Contact Us</h4>
                <ul className="space-y-4 text-gray-400 text-sm">
          ...
          <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500">
            <p>© 2025 Silk Oak Hotels. All Rights Reserved.</p>
            <div className="flex items-center space-x-6 mt-4 md:mt-0">
               <a href="#" className="hover:text-gold-400 transition-colors">Privacy Policy</a>
               <a href="#" className="hover:text-gold-400 transition-colors">Terms</a>
               <span className="text-gray-700">|</span>
               <p>
                 Designed by <a href="https://novus-studios.vercel.app" target="_blank" rel="noopener noreferrer" className="text-gold-500 hover:text-white transition-colors font-medium">Novus Studios</a>
               </p>
            </div>
          </div>
        </div>
